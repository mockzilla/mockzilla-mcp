...
docs/          Generated by scripts/sync-docs.mjs. Never edit by hand.
...
- **Product docs.** Django's `publish_docs` writes `export/mcp.json` to the
  docs bucket. `make docs` copies it and rebuilds. Edit the docs in the Django
  admin, never here.
...
A docs change reaches agents with the next release: publish the docs, run
`make docs`, commit, release. To preview unpublished docs, run
`make docs-mcp-dump` in the django repo, then `make docs-local` here.
