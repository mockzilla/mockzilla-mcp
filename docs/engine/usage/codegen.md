...
Use codegen mode when you need more than what Portable Mode (topic `engine/usage/portable`) offers -
...
Edit `middleware.go` to add authentication, logging, or request modification. See Custom Middleware (topic `engine/middleware`) for details and examples.
...
Each service has a `setup/config.yml` for runtime behavior. See Service Config (topic `engine/config/service`) for all options:
...
Control what gets generated via `setup/codegen.yml`. See Codegen Config (topic `engine/config/codegen`) for details.

## Learn More

- Service Config (topic `engine/config/service`) - latency, errors, upstream proxy, caching, replay
- Codegen Config (topic `engine/config/codegen`) - code generation settings
- Custom Middleware (topic `engine/middleware`) - authentication, logging, request modification
- Contexts (topic `engine/contexts`) - control generated values with fake data, patterns, aliases
- Replay (topic `engine/replay`) - record and replay API responses
- Factory (topic `engine/factory`) - use the generation engine programmatically from Go
- Service Command (topic `engine/commands/service`) - full reference for `gen/service`
...
| Need | Use |
|------|-----|
| Quick prototyping, testing | Portable Mode (topic `engine/usage/portable`) |
