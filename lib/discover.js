...
export async function discoverSpecs(args) {
  ...
  const found = await listSpecFiles(dir);
  // A spec directory can hold hundreds of files. Summarising all of
  // them spawns one `mockzilla info` each and returns a payload far
  // past what an agent can read, so cap the detail and say so.
  const specFiles = await mapLimit(
    found.slice(0, SPEC_SUMMARY_MAX),
    SUMMARY_CONCURRENCY,
    summariseSpec,
  );
  ...
  if (found.length > specFiles.length) {
    out.spec_file_count = found.length;
    out.truncated = true;
    out.notes =
      `${found.length} spec files here; the first ${specFiles.length} are ` +
      "summarised. Pass a single file to `info`, or a narrower `dir`.";
  }
  if (services.length === 0 && found.length === 0 && !hasStatic) {
    const nested = await childDirsWithSpecs(dir);
    out.notes =
      nested.length > 0
        ? "Nothing mockzilla can serve at this level, but these " +
          `subdirectories hold specs: ${nested.join(", ")}. Call ` +
          "discover_specs on one of them."
        : "Nothing mockzilla can serve here: no spec files, no services/ " +
          "folder and no static index.<ext> files.";
  }
  return out;
}

const SPEC_SUMMARY_MAX = 40;
const SUMMARY_CONCURRENCY = 8;
const NESTED_HINT_MAX = 10;
...
// An empty result on a tree like specs/3.0 is a dead end for the
// agent; name the children that do hold specs so it can recurse.
async function childDirsWithSpecs(dir) {
  const entries = await readdir(dir, { withFileTypes: true }).catch(() => []);
  const hits = [];
  for (const e of entries) {
    if (!e.isDirectory() || shouldSkipDir(e.name)) continue;
    const child = path.join(dir, e.name);
    const specs = await listSpecFiles(child).catch(() => []);
    if (specs.length > 0) hits.push(e.name);
    if (hits.length >= NESTED_HINT_MAX) break;
  }
  return hits;
}
...
async function summariseSpec(file) {
  ...
  try {
    ...
    return {
      path: file,
      error: briefError((err.stderr && err.stderr.trim()) || err.message),
    };
  }
}
