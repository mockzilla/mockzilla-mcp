...
export async function publishToGithub(args = {}) {
  ...
  try {
    ...
    // Teardown is a workflow of its own, so an existing publish workflow
    // is never touched to make it possible.
    const removeFile = path.join(work, ".github", "workflows", REMOVE_WORKFLOW_NAME);
    const workflowPaths = (await listFiles(path.join(work, ".github", "workflows")))
      .map((f) => path.relative(work, f));
    const teardown = await findTeardownWorkflow(workflowPaths, (f) =>
      readText(path.join(work, f)),
    );
    if (!teardown) {
      await mkdir(path.join(work, ".github", "workflows"), { recursive: true });
      await writeFile(removeFile, REMOVE_WORKFLOW);
    }
    const teardownReady = true;
    ...
    return {
      ...
      notes: [
        ...
        hadWorkflow
          ...
          : null,
        teardown
          ? `Teardown already possible through ${teardown.file}.`
          : `Added ${REMOVE_WORKFLOW_NAME}, which unpublish_from_github runs ` +
            "to free the simulation slot.",
...
export async function checkGithubDeployable(args = {}) {
  ...
  if (hasRepo) {
    ...
    const readRemote = (p) =>
      run(["gh", "api", `repos/${repo}/contents/${p}`, "--jq", ".content"])
        ...
        .catch(() => null);
    workflowText = await findMockzillaWorkflow(paths, readRemote);
    teardown = await findTeardownWorkflow(paths, readRemote);
  } else {
    ...
    const readLocal = (p) => readText(path.join(root, p));
    workflowText = await findMockzillaWorkflow(paths, readLocal);
    teardown = await findTeardownWorkflow(paths, readLocal);
  }
  ...
  if (isCodegen) {
    return codegenReport({ target, paths, workflowText, codegen, hasRepo, repo: args.repo, teardown });
  }
  ...
  if (!workflowText) {
    problems.push({
      ...
      fix:
        `Add ${WORKFLOW_PATH}. \`workflow_to_add\` holds a complete one, ` +
        "with the manual trigger that makes teardown possible.",
    });
  } else {
    ...
    if (teardown) {
      ok.push(`${teardown.file} can take the mocks down`);
    } else {
      warnings.push({
      what: "Nothing can take the mocks down",
        why:
          "The action removes a repository's mocks only on a run with " +
          "`delete: true`, and push and pull_request triggers cannot pass " +
          "an input, so there is no way to free the simulation slot short " +
          "of editing the workflow, pushing, and editing it back.",
        fix: teardownFix(workflowText),
      });
    }
  }
  ...
  return {
    target,
    services_dir: servicesRel,
    teardown_workflow: teardown ? teardown.file : null,
    services,
    ...
    teardown_workflow_to_add: teardown ? null : REMOVE_WORKFLOW,
    notes: [
      problems.length
...
// Teardown works through any workflow that runs the action by hand with
// delete set: a dedicated remove workflow, or the publish one carrying a
// delete input. Look for either rather than assuming one shape.
async function findTeardownWorkflow(paths, read) {
  const candidates = paths
    .filter((p) => /^\.github\/workflows\/.+\.ya?ml$/i.test(p))
    .sort((a, b) => Number(b.includes("remove")) - Number(a.includes("remove")))
    .slice(0, WORKFLOW_SCAN_MAX);
  for (const p of candidates) {
    const text = await read(p);
    if (!text || !/mockzilla\/actions/.test(text)) continue;
    if (!/workflow_dispatch/.test(text)) continue;
    if (/delete\s*:\s*true/.test(text)) return { file: path.basename(p), input: false };
    if (/inputs\.delete|delete\s*:\s*\n?\s*(type|description)/.test(text)) {
      return { file: path.basename(p), input: true };
    }
  }
  return null;
}
...
// The documented way is a workflow of its own, and it is the simpler
// one: nothing to wire through the publish workflow, and no risk of the
// job's `if:` guard skipping the manual run, which is what makes
// retrofitting the publish workflow go quietly wrong.
const REMOVE_WORKFLOW_NAME = "mockzilla-remove.yml";
const REMOVE_WORKFLOW = `name: mockzilla-remove
...
function teardownFix(workflowText) {
  const guarded =
    /^\s*if\s*:/m.test(workflowText || "") &&
    /github\.event_name/.test(workflowText || "");
  return (
    "Add a second workflow, .github/workflows/mockzilla-remove.yml, holding " +
    "`teardown_workflow_to_add`. It triggers only by hand and passes " +
    "`delete: true`, so the publish workflow is left alone. Then: " +
    "gh workflow run mockzilla-remove.yml" +
    (guarded
      ? " Retrofitting the publish workflow instead is the fiddlier route " +
        "here: its job has an `if:` on github.event_name, and a " +
        "workflow_dispatch run matches none of those, so the job would be " +
        "skipped and the trigger would look broken."
      : "")
  );
}
...
function codegenReport({ target, workflowText, codegen, hasRepo, repo, teardown }) {
  const problems = [];
  const warnings = [];
  const ok = [];
...
export async function unpublishFromGithub(args = {}) {
  ...
  // A repository can tear down through a dedicated remove workflow or
  // through the publish one carrying a delete input. Find which, rather
  // than assuming a file name and an input that may not exist.
  const tree = await run([
    "gh", "api", `repos/${repo}/git/trees/HEAD?recursive=1`,
    "--jq", ".tree[] | select(.type==\"blob\") | .path",
  ]).catch(() => "");
  const teardown = await findTeardownWorkflow(
    tree.split("\n").map((l) => l.trim()).filter(Boolean),
    (f) =>
      run(["gh", "api", `repos/${repo}/contents/${f}`, "--jq", ".content"])
        .then((b64) => Buffer.from(b64.replace(/\s/g, ""), "base64").toString("utf8"))
        .catch(() => null),
  );
  ...
  if (!teardown) {
    throw new Error(
      `${repo} has no workflow that can take its mocks down. The action ` +
        "removes them only on a manual run with `delete: true`, and nothing " +
        "here triggers that. Add .github/workflows/mockzilla-remove.yml " +
        "(check_github_deployable returns one in " +
        "`teardown_workflow_to_add`), or run the action with delete: true " +
        "by hand." +
        (args.delete_repo === true
          ? " Deleting the repository alone does NOT free the simulation slot."
          : ""),
    );
  }

  const dispatched = await run([
    "gh", "workflow", "run", teardown.file, "--repo", repo,
    ...(teardown.input ? ["-f", "delete=true"] : []),
  ]).then(
    () => true,
    () => false,
  );
  ...
  return {
    repo,
    teardown_workflow: teardown.file,
    teardown_dispatched: dispatched,
    repo_deleted: repoDeleted,
    notes: [
      dispatched
        ? `Started ${teardown.file}, which runs the action with delete: true ` +
          "and frees this repository's simulation slot. It takes a few seconds; " +
          "the run shows under the repository's Actions tab."
        : `Could not start ${teardown.file}. It may not be enabled on the ` +
          "default branch yet: a workflow_dispatch trigger only becomes " +
          "available once the workflow is on it.",
...
// Two files, which is what the action's own docs describe. Keeping
// teardown in its own workflow means the publish one needs no delete
// plumbing, and no `if:` guard can skip the manual run.
function workflowYaml(servicesRel = "services") {
  const servicesInput =
    servicesRel === "services" ? "" : `\n          services-dir: ${servicesRel}`;
  return `name: mockzilla

# Written by mockzilla-mcp. Teardown lives in mockzilla-remove.yml,
# which runs the same action with delete: true.
on:
  push:
  ...
  publish:
    ...
    steps:
      ...
      - uses: mockzilla/actions@v1
        with:
          token: \${{ secrets.GITHUB_TOKEN }}${servicesInput}
`;
}
