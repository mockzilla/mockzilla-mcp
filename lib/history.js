...
export async function diagnoseRequests(args = {}) {
  ...
  return {
    ...
    // Without this an agent reads a history of nothing but `mock` and
    // concludes replay is not working, when replay is in fact serving
    // every call.
    not_counted:
      "A replayed or cached response short-circuits before the history " +
      "middleware, so neither appears here at all and none of the counts " +
      "above include them. A request missing from history was most likely " +
      "served from a replay recording or the cache. Call the endpoint with " +
      "call_endpoint and read X-Mockzilla-Source to see which, or " +
      "list_replays to see what is pinned.",
  };
}
...
// Where the data came from. Only two answers are possible: replayed
// and cached responses never reach the history middleware, so every
// recorded entry was either fetched from an upstream or generated
// here. isFromUpstream is the field to read, and it is right on every
// engine version; the stored X-Mockzilla-Source header was the
// upstream's own tag rather than ours before 2.8.21.
