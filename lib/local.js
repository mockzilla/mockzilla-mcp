...
export async function mockEndpoint(args) {
  ...
  // A static endpoint is a bare body file, and the engine synthesizes
  // its spec with a single 200 response, so there is nowhere to put a
  // status. Refusing beats writing the file and reporting a status the
  // server will not send.
  if (args.status !== undefined && args.status !== 200) {
    throw new Error(
      `mock_endpoint cannot serve status ${args.status}: a static endpoint ` +
        "always answers 200 with its body. Options: (a) keep the body and " +
        "drop `status`, returning 200; (b) to exercise a client's error " +
        "path, call serve_locally with `errors` (e.g. {\"p100\": " +
        `${args.status}}), which returns ${args.status} for every request ` +
        "but replaces the body with \"Simulated error\"; (c) for a real " +
        "status-plus-body pair, write an OpenAPI spec and use serve_locally.",
    );
  }
  const response =
