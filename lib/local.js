...
import { MOCKZILLA_VERSION, resolveMockzilla } from "./install.js";
import { briefError, shellEscape, versionAtLeast } from "./util.js";

const exec = promisify(execCb);

const READY_TIMEOUT_MS = 30_000;

// --errors parsed but never reached the percentile table before this
// release, so an older CLI accepts the flag and injects nothing.
const ERRORS_MIN_VERSION = "2.8.17";

// meta.json carries a static response's status and headers. An older
// CLI has no such convention and serves the file as a literal asset.
const META_MIN_VERSION = "2.8.20";
...
// mockzilla's multi-service layout: each services/<name>/ mounts at /<name>.
const MOCKS_SERVICES_DIR = path.join(MOCKS_ROOT, "services");
...
export async function serveLocally(args) {
  ...
  const extraArgs = serviceFlagArgs(args, inputs);

  if (args.errors !== undefined) await assertErrorsSupported();
  ...
  let stamp;
  try {
    stamp = await spawnPortable({ inputs, port, kind: "adhoc", extraArgs });
  } catch (err) {
    ...
    if (port === MANAGED_DEFAULT_PORT && !Number.isInteger(args.port)) {
      stamp = await spawnPortable({ inputs, port: 0, kind: "adhoc", extraArgs });
    } else {
      throw err;
    }
  }
  return {
    ...
    via: stamp.via,
    // Without these the agent guesses a path from the spec's filename
    // and gets a 404. Mounts are prefixes, so the spec's own paths are
    // not callable URLs on their own.
    ...(await exampleEndpoints(inputs, stamp)),
...
export async function mockEndpoint(args) {
  ...
  // A meta.json the engine rejects takes its whole service down with
  // it, so validate before anything is written, not after.
  if (meta) await assertMetaSupported();

  // Omitting the body alongside a status writes meta.json on its own,
  // which the engine serves as a response with no body: the only way
  // to express 204 or 304.
  const bodyless = meta !== null && args.response === undefined;
  const response =
  ...
  const file = bodyless ? null : path.join(dir, `index.${ext}`);
  ...
  if (file) {
    ...
    // A leftover body from an earlier call would keep being served.
    await rm(path.join(dir, `index.${ext}`), { force: true });
  }
...
export async function listMockEndpoints() {
  const endpoints = [];
  await moveLegacyMocks();
  const rootStat = await stat(MOCKS_SERVICES_DIR).catch(() => null);
  ...
  const services = await readdir(MOCKS_SERVICES_DIR, { withFileTypes: true });
  for (const svc of services) {
    if (!svc.isDirectory()) continue;
    await collectEndpoints(
      path.join(MOCKS_SERVICES_DIR, svc.name),
      svc.name,
      [],
      endpoints,
    );
...
// Mocks used to live under static/, which mockzilla serves as one service at
// /static, so every returned URL was off by that prefix. Move them once.
async function moveLegacyMocks() {
  const legacy = path.join(MOCKS_ROOT, "static");
  const legacyStat = await stat(legacy).catch(() => null);
  if (!legacyStat?.isDirectory()) return;
  if (await stat(MOCKS_SERVICES_DIR).catch(() => null)) return;
  await killManaged();
  await rename(legacy, MOCKS_SERVICES_DIR);
}
...
async function collectEndpoints(dir, service, pathSegs, out) {
  ...
  for (const e of entries) {
    ...
    if (ALLOWED_METHODS.has(e.name.toUpperCase())) {
      ...
      // meta.json alone is a real endpoint: a response with no body.
      const hasMeta = childEntries.some(
        (ce) => ce.isFile() && ce.name === "meta.json",
      );
      if (idx || hasMeta) {
        const meta = hasMeta
          ? await readMeta(path.join(childDir, "meta.json"))
          : null;
        out.push({
          ...
          ...(idx ? {} : { body: null }),
          file: idx ? path.join(childDir, idx.name) : null,
        });
      }
    } else {
...
// The running server, whichever tool started it. The history and
// replay APIs are on the same origin as the mocks.
export function runningServer() {
...
export async function lint(args) {
  ...
  const base = resolved.type === "binary" ? [resolved.path] : resolved.invocation;
  if (!(await cliHasLint(base, resolved))) {
    throw new Error(
      `mockzilla ${resolved.version} (${resolved.source}) has no lint ` +
        `command. It needs ${MOCKZILLA_VERSION} or newer: upgrade the ` +
        "installed CLI, or call install_cli if it came from the bridge cache.",
    );
  }
  const cmdline = [...base, "lint", "--format", "json", args.input].map(shellEscape).join(" ");

  let stdout;
  try {
    ({ stdout } = await exec(cmdline));
  } catch (err) {
    // Exit 1 with a JSON report on stdout means defects, not a failure.
  ...
  const defects = JSON.parse(stdout).defects ?? [];
  return {
    clean: defects.length === 0,
    defect_count: defects.length,
    defects: defects.slice(0, LINT_DEFECTS_MAX),
    truncated: defects.length > LINT_DEFECTS_MAX,
  };
}

const lintSupport = new Map();

// Older CLIs read `lint` as a path to serve, which can start a server
// instead of failing, so check the usage text before running it.
async function cliHasLint(base, resolved) {
  const key = `${resolved.source}:${resolved.version}`;
  if (!lintSupport.has(key)) {
    const { stdout } = await exec([...base, "--help"].map(shellEscape).join(" "));
    lintSupport.set(key, /^\s*mockzilla lint /m.test(stdout));
  }
  return lintSupport.get(key);
}
...
async function spawnPortable({ inputs, port, kind, extraArgs = [] }) {
  if (starting) {
    throw new Error(
  ...
  try {
    ...
    const cliArgs = ["--ready-stamp", "--port", String(port), ...extraArgs, ...inputs];
    const [cmd, ...prefix] =
      resolved.type === "binary" ? [resolved.path] : resolved.invocation;
    ...
    const recentLines = [];
    const ready = waitForReadyStamp(child, recentLines);
    ready.finally(() => {
      child.stdout?.removeAllListeners("data");
      child.stdout?.resume();
    });

    // "close", not "exit": it fires after stdio drains, so the CLI's own
    // error line is already in recentLines.
    const exited = new Promise((_, reject) => {
      child.once("close", (code, signal) => {
        const reason = exitReason(recentLines);
        reject(
          new Error(
...
function waitForReadyStamp(child, recentLines) {
...
function rememberLine(recentLines, line) {
...
function exitReason(recentLines) {
  const errors = recentLines.filter((l) => /\bERR(OR)?\b/.test(l));
  return (errors.length > 0 ? errors : recentLines).slice(-3).join(" | ");
}

const GO_DURATION = /^(\d+(\.\d+)?(ns|us|µs|ms|s|m|h))+$/;
const ERROR_RULE_KEY = /^p([1-9]\d?|100)$/;

// The CLI applies these to a single service only. Checking here gives the
// agent a clear message instead of a failed spawn.
function serviceFlagArgs(args, inputs) {
  const out = [];
  if (args.latency !== undefined) {
    if (typeof args.latency !== "string" || !GO_DURATION.test(args.latency)) {
      throw new Error("`latency` must be a duration like \"100ms\" or \"1.5s\"");
    }
    out.push(`--latency=${args.latency}`);
  }
  if (args.errors !== undefined) {
    const rules = args.errors;
    if (rules === null || typeof rules !== "object" || Array.isArray(rules)) {
      throw new Error("`errors` must be an object like {\"p5\": 500, \"p10\": 503}");
    }
    const pairs = [];
    for (const [key, status] of Object.entries(rules)) {
      if (!ERROR_RULE_KEY.test(key)) {
        throw new Error(`\`errors\` key ${JSON.stringify(key)} must be p1 to p100`);
      }
      if (!Number.isInteger(status) || status < 100 || status > 599) {
        throw new Error(`\`errors.${key}\` must be an HTTP status from 100 to 599`);
      }
      pairs.push(`${key}=${status}`);
    }
    if (pairs.length > 0) out.push(`--errors=${pairs.join(",")}`);
  }
  if (args.mount !== undefined) {
    if (typeof args.mount !== "string" || args.mount.length === 0) {
      throw new Error("`mount` must be a non-empty string");
    }
    out.push(`--mount=${args.mount}`);
  }
  if (args.context !== undefined) {
    if (typeof args.context !== "string" || args.context.length === 0) {
      throw new Error("`context` must be a path to a context YAML file");
    }
    out.push(`--context=${args.context}`);
  }
  if (out.length > 0 && inputs.length !== 1) {
    throw new Error(
      "`latency`, `errors`, `mount` and `context` apply to one service, " +
        `so \`input\` must be a single spec or folder (got ${inputs.length}).`,
    );
  }
  return out;
}

const EXAMPLE_ENDPOINTS_MAX = 8;

// Turn the served spec's paths into URLs that actually answer. Only
// for one spec mounted as one service: with several, which input
// produced which mount is guesswork, and a wrong URL is worse than none.
async function exampleEndpoints(inputs, stamp) {
  if (inputs.length !== 1 || stamp.services.length !== 1) {
    return {
      endpoints_hint:
        "Several services are mounted. Call call_endpoint on " +
        `${stamp.url.replace(/\/$/, "")}/.services for the mounts, and ` +
        "`info` on an input for its paths. A URL is <mount>/<spec path>.",
    };
  }
  const svc = stamp.services[0];
  const prefix = `${stamp.url.replace(/\/$/, "")}${svc.path}`;
  const spec = await runInfo(inputs[0]).catch(() => null);
  const paths = Array.isArray(spec?.paths) ? spec.paths : [];
  if (paths.length === 0) return { endpoints_hint: null };
  return {
    endpoint_count: paths.length,
    example_endpoints: paths.slice(0, EXAMPLE_ENDPOINTS_MAX).map((p) => ({
      method: p.method,
      url: `${prefix}${p.path}`,
    })),
    endpoints_hint:
      `Every URL is ${prefix} + the spec's path` +
      (paths.length > EXAMPLE_ENDPOINTS_MAX
        ? `. ${paths.length - EXAMPLE_ENDPOINTS_MAX} more: call \`info\` on the input for all of them.`
        : ".") +
      " Substitute a real value for any {placeholder}.",
  };
}

// Returns the meta.json contents, or null when the defaults (200, no
// extra headers) already describe the response and no file is needed.
function buildMeta(args) {
  const meta = {};
  if (args.status !== undefined) {
    if (!Number.isInteger(args.status) || args.status < 200 || args.status > 599) {
      throw new Error("`status` must be an integer from 200 to 599");
    }
    if (args.status !== 200) meta.status = args.status;
  }
  if (args.headers !== undefined) {
    const h = args.headers;
    if (h === null || typeof h !== "object" || Array.isArray(h)) {
      throw new Error("`headers` must be an object of header name to string");
    }
    ...
    if (entries.length > 0) meta.headers = Object.fromEntries(entries);
  }
  return Object.keys(meta).length > 0 ? meta : null;
}

async function assertMetaSupported() {
  const resolved = await resolveMockzilla();
  if (!resolved) {
    throw new Error("mockzilla is not installed. Call install_cli first.");
  }
  if (versionAtLeast(resolved.version, META_MIN_VERSION)) return;
  throw new Error(
    `mockzilla ${resolved.version} (${resolved.source}) cannot set a status ` +
      `or headers on a static response: meta.json arrived in ${META_MIN_VERSION}. ` +
      `Upgrade the installed CLI to ${MOCKZILLA_VERSION} or newer, or call ` +
      "install_cli to use the bridge's own copy. Retry without `status` and " +
      "`headers` to mock a 200.",
  );
}

async function assertErrorsSupported() {
  const resolved = await resolveMockzilla();
  if (!resolved) {
    throw new Error("mockzilla is not installed. Call install_cli first.");
  }
  if (versionAtLeast(resolved.version, ERRORS_MIN_VERSION)) return;
  throw new Error(
    `mockzilla ${resolved.version} (${resolved.source}) accepts \`errors\` ` +
      "but injects nothing: the flag had no effect before " +
      `${ERRORS_MIN_VERSION}. Upgrade the installed CLI to ` +
      `${MOCKZILLA_VERSION} or newer, or call install_cli to use the ` +
      "bridge's own copy. Retry without `errors` to serve normally.",
  );
}
