...
// Each candidate is probed for availability before we hand it to
// mockzilla, then the actual call has a final retry-on-zero in case
// something grabbed the port in the race window.
async function spawnManagedServer() {
  const candidates = [];
  if (lastManagedPort) candidates.push(lastManagedPort);
  if (
    Number.isInteger(MANAGED_DEFAULT_PORT) &&
    MANAGED_DEFAULT_PORT > 0 &&
    !candidates.includes(MANAGED_DEFAULT_PORT)
  ) {
    candidates.push(MANAGED_DEFAULT_PORT);
  }
  ...
  for (const port of candidates) {
    if (await isPortFree(port)) {
      try {
        ...
        // Race: someone grabbed the port between probe and bind.
        // Keep trying the rest of the list, then fall back to 0.
      }
    }
  }
